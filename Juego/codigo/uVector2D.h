...
class Vector2D
{
  ...
  inline float AnguloDEG()const;
  inline float AnguloRAD()const;  
...
inline float Vector2D::AnguloDEG()const
{
    if ( x == 0 )
    {
        return (y<0)?-90:90 ;
    }
    return RADTODEG( atan( y/x ) ) + ((x<0)?180:0);  
}

inline float Vector2D::AnguloRAD()const
{
	if ( x == 0 )
    {
